...
# Function to call Gemini and extract raid days
def get_raid_days(guild_description):
    ...
    try:
        model = genai.GenerativeModel("gemini-2.0-flash-lite-preview-02-05")  # Use Gemini Pro model
...
for index, row in df.iterrows():
    ...
    # Call Gemini API to analyze raid days
    ...
    remaining_time = 2 - elapsed_time  # Time left to reach 2 seconds

    # Ensure the request interval is at least 2 seconds
    if remaining_time > 0:
        time.sleep(remaining_time)  # Sleep only if needed
...
output_csv = 'wowprogress_guilds_filtered_with_raiddays_gemini.csv'
